swt_markers_sys_create = {
	...
	*/
	...
	if (_Type != -2) then {_mark setMarkerTypeLocal (swt_cfgMarkers_names select _Type)};
	_mark setMarkerColorLocal (swt_cfgMarkerColors_names select _Color);
	if (_Type != -2) then {_mark setMarkerTextLocal _Text};
	_mark setMarkerDirLocal _Dir;
	if (_Type != -2) then {_mark setMarkerSizeLocal [_Scale,_Scale]} else {_mark setMarkerSizeLocal [5,_Scale]};
	if (_Type == -2) then {
...
swt_markers_sys_sendMark = compile preprocessFileLineNumbers '\swt_markers_a3\swt_markers_sys_sendMark.sqf';
...
swt_sys_del = {
    ...
    /*if ((_this select 1)==15) then {
    	swt_markers_bis_markers = !swt_markers_bis_markers;
    	hint (if (swt_markers_bis_markers) then {"SWT MARKERS: TAB, Changed to BIS markers system"} else {"SWT MARKERS: TAB, Changed to sweet markers system"});
    };*/
...
swt_sys_pos_m = {
	...
	if ((swt_markers_fast_way)and(((swt_markers_fast_enabled)and(((swt_markers_fast_enabled_for) and (((leader player == player) or (((effectiveCommander (vehicle player)) == player) and (vehicle player != player))))) or (!swt_markers_fast_enabled_for)))or !isMultiplayer)) then
	{
	...
	if !(isNil "swt_markers_line_params_world") then {
		...
		swt_markers_line_params_world set [3, ((swt_markers_line_params_world select 0) distance (swt_markers_line_params_world select 1))/2];
		"SWT_MARKERS LOCAL LINE" setMarkerSizeLocal [5,(((swt_markers_line_params_world select 0) distance (swt_markers_line_params_world select 1)))/2];
