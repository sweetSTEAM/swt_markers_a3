...
swt_load_comboboxes = {
	disableSerialization;
	private ['_display'];
	if (_this isEqualTo 0) then {_display = findDisplay 54} else {_display = _this};
	_combo_color = [2100,2101,2102,2103,2104,2105];
	_combo_icon = [2106,2107,2108,2109,2110,2111];
	...
	switch (_this select 1) do {
		...
		case 'SHOW INFO': {
			...
			if (!isnil "RscDisplayInsertMarker_info") then {
				((findDisplay 54) displayctrl 2400) call compile preprocessfilelinenumbers '\swt_markers_a3\RscDisplayInsertMarker.sqf';
				uiSleep 0.25;
			};
...
disableSerialization;
	_pos = ctrlPosition (_display displayCtrl 204);//[x,y,w,h]
...
swt_start_dir = {
    ...
    if (!_shift and !_alt and _ctrlKey and((_this select 1) == 0)) then {
    	displayNull call swt_markers_sys_sendMark;
    };
	...
	if (!_shift and _ctrlKey and _alt and ((_this select 1) == 0)) then {
    	swt_markers_line_params_world = [(_display displayCtrl 51) ctrlMapScreenToWorld _pos_click,(_display displayCtrl 51) ctrlMapScreenToWorld _pos_click,0,0];
    	...
    	"SWT_MARKERS LOCAL LINE" setMarkerSizeLocal [5,0];
    };

	if (_shift and _ctrlKey and !_alt and ((_this select 1) == 0) and ((leader player == player) or (effectiveCommander (vehicle player) == player))) then {
		if (isNil "swt_markers_way_first") then {
			...
			if (isNil {(swt_markers_way_first_pos nearRoads 16) select 0}) exitWith {hint "SWT MARKERS: ROAD NOT FOUND"};
			swt_markers_way_first = true;
			hint "SWT MARKERS: FIRST WAYPOINT";
		} else {
			swt_markers_way_second_pos = (_display displayCtrl 51) ctrlMapScreenToWorld swt_markers_pos_m;
			if (isNil {(swt_markers_way_second_pos nearRoads 16) select 0}) exitWith {hint "SWT MARKERS: Road not found"};
			//hint "SWT MARKERS: SECOND WAYPOINT";
			swt_markers_way_first = nil;
			[swt_markers_way_first_pos,swt_markers_way_second_pos] call swt_markers_legendagy;
		};
	};
...
swt_mouse_up = {
	if !(isNil "swt_mark_to_change_dir") then {
		swt_change_dir_PV = [swt_mark_to_change_dir,swt_markers_direction,player];
	...
	if !(isNil "swt_markers_line_params_world") then {
		...
		{
			swt_markers_line_params_world call swt_markers_sys_sendMark;
		};
...
//Короче, я запутался, всё.
	...
	_first_road = (_first_pos nearRoads 20) select 0;
	_second_road = (_second_pos nearRoads 20) select 0;
	_loop = true;
	_g = 0;
	_swt_black_roads = [_first_road];
	_swt_way_roads = [_first_road];
	while {_loop} do {
		if (_first_road isEqualTo _second_road) exitWith {hint "done"; _loop = false};
	    _roads = ((position _first_road) nearRoads _rdischeck) - _swt_black_roads;
    	if ((_roads isEqualTo []) or (count _roads < 3)) then {
    		_exit = false;
    		for "_k" from 2 to 6 step 2 do {
    			_roads = ((position _first_road) nearRoads (_rdischeck + 5*_k)) - _swt_black_roads;
    			if !(_roads isEqualTo []) exitWith {_exit = true};
    		};
    		if !(_exit) then {
    			_step = 0;
    			_roads1 = [];
		    	for "_i" from ((count _swt_black_roads)-1) to 0 step -1 do {
		    		//if (!isNil {(swt_black_roads select _i)}) then {
		    			if (_step > 30) exitWith {hint (localize "STR_SWT_M_MESS_WAYERROR");
		    		systemChat (localize "STR_SWT_M_MESS_WAYERROR");breakOut "swt_loop";_swt_way_roads = [];};
		    			_step = _step + 1;
			    		_roads1 = _roads1 + ((_swt_black_roads select _i) nearRoads 40);
			    		_roads = ((_swt_black_roads select _i) nearRoads 40) - swt_black_roads;
			    		if !(_roads isEqualTo []) exitWith {_swt_way_roads = _swt_way_roads - _roads1};
		    		//};
		    	};
		    	if (_roads isEqualTo []) exitWith {hint (localize "STR_SWT_M_MESS_WAYERROR");
		    		systemChat (localize "STR_SWT_M_MESS_WAYERROR");breakOut "swt_loop";_swt_way_roads = [];};
		    };
    	};
    	//if (count _roads > 1) then {
		    _fs = [];
			{
				_fs pushBack ((_x distance _second_road) + (_x distance _first_road));// + abs(abs(getDir _first_road) - abs(getDir _first_road))*0.15);
				//Условие нужно доработать
		...
		_swt_black_roads pushBack _first_road;
		_swt_way_roads pushBack _first_road;
	};

	{
		(getPosATL _x) call swt_markers_sys_sendMark; //ПЕРЕДЕЛАТЬ
	} forEach _swt_way_roads;
...
uiNamespace setVariable ["swt_displ",_mapDisplay];
_id0 = (_mapDisplay displayCtrl 51) ctrlAddEventHandler ["MouseButtonDblClick", "if ((_this select 1) == 0) then {swt_markers_display_coord = [(_this select 2),(_this select 3)]};"];
_id1 = (_mapDisplay displayCtrl 51) ctrlAddEventHandler ["MouseButtonDown", "_this call swt_start_dir"];
_id2 = (_mapDisplay displayCtrl 51) ctrlAddEventHandler ["MouseButtonUp", "_this call swt_mouse_up"];
_id3 = (_mapDisplay) displayAddEventHandler ["KeyDown", "_this call swt_sys_del"];
//_id3_1 = (_mapDisplay) displayAddEventHandler ["KeyUp", "if ((_this select 1)==) then {swt_markers_tab = false};"];
...
sleep 1;
(_mapDisplay displayCtrl 51) ctrlRemoveEventHandler ["MouseButtonDblClick",_id0];
(_mapDisplay displayCtrl 51) ctrlRemoveEventHandler ["MouseButtonDown",_id1];
(_mapDisplay displayCtrl 51) ctrlRemoveEventHandler ["MouseButtonUp",_id2];
...
diag_log format ["SWT MARKERS: MY MAP DISPLAY %1",_mapDisplay];
uiNamespace setVariable ["swt_displ2",_mapDisplay];
_id0 = (_mapDisplay displayCtrl 51) ctrlAddEventHandler ["MouseButtonDblClick", "if ((_this select 1) == 0) then {swt_markers_display_coord = [(_this select 2),(_this select 3)]};"];
_id1 = (_mapDisplay displayCtrl 51) ctrlAddEventHandler ["MouseButtonDown", "_this call swt_start_dir"];
_id2 = (_mapDisplay displayCtrl 51) ctrlAddEventHandler ["MouseButtonUp", "_this call swt_mouse_up"];
_id3 = (_mapDisplay) displayAddEventHandler ["KeyDown", "_this call swt_sys_del"];
